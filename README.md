...
表單管理只出現在教師端的「表單」分頁。新題目預設為選擇題，每題提供至少兩個選項，也可自行新增選項或題目。教師先「儲存草稿」，可隨時按「修改草稿」與預覽；草稿不會被家長讀取。確認後按「發佈到班群」，系統才會公告並開放填寫，為避免已填回覆與題目不一致，發佈後不能再修改題目。按「預覽」可在儲存或發佈前直接查看家長端的填寫畫面；預覽不會建立表單、公告或儲存任何資料。
