...
- 「查詢」分頁可查看老師建立的資料；資料可設定為全班可查，或只允許目前登入家長查看自己的內容。
...
- 班級事項可讀取 [807 班級看板](https://github.com/qfm-11407/classboard-807) 的下一個上課日「明日事項」；匯入前可勾選、修改與調整順序，確認後只附加到聯絡簿編輯區，仍須由教師自行儲存發布。
- 聯絡簿集中顯示待簽、已簽、未簽與請假統計；未簽採紅色提示，可篩選名單並快速私訊。
- 「調查」分為「表單」與「資料」；資料分頁可建立家長查詢項目、設定「全班可查」或「僅限本人」、以試算表格式批次貼上個人資料，並保留教師專用的全班欄位查詢與學生個人資料卡。個人資料卡可顯示指定相簿資料夾內的學生大頭照縮圖。
...
| Google Apps Script | 統一登入、教師／家長權限判斷、核發短效 Firebase 自訂憑證、圖片上傳、學校行事曆與班級看板明日事項讀取，以及語音通話訊號交換。 |
| Google 試算表 | 保存家長帳號、驗證碼、學生名單，以及教師限定的學生個人資料。 |
| Google Drive | 保存班級相簿、聊天圖片、作業圖片，以及按學生與日期歸檔的私密請假簽名。 |
...
電腦可直接開啟 [教師相簿牆](https://suyungsheng-qfm.github.io/class-contact-book/gallery.html)。首次開啟會要求教師透過統一登入頁驗證；登入完成後，頁面會依既有相簿日期資料夾顯示照片牆，並提供座號／檔名搜尋、「開啟」Google Drive 原始檔與「下載」連結。日期可多選，也可切換為「排除勾選日期」；為避免一次載入過多縮圖，單次最多顯示 12 個日期。這個網址不放在教師端的功能列，方便在電腦另開分頁使用。
...
教師在「聯絡簿 → 作業資料」填入標題，可一次選多張圖片；完成後班群只會出現一張「查看作業」卡片。家長必須先登入，才能透過卡片開啟作業資料頁。每張圖片可填寫說明，並以向上／向下按鈕調整顯示順序；家長頁會依相同順序顯示說明。已發佈的作業可按「編輯」修改標題、科目、說明與截止日期，並可**追加**多張圖片；班群保留原本同一張入口卡片，不會重新發送。教師也能按「收回」，立即關閉家長頁並將班群卡片改為「訊息已收回」。

作業圖片會放在班級相簿根資料夾下的 `作業資料 / 作業識別碼` 資料夾。為了讓各型手機的網頁能直接載入圖片，這些**圖片檔案**採「知道連結者可檢視」；請勿上傳學生個資、成績、聯絡資訊或任何不適合被轉傳的內容。作業標題、說明與入口頁本身仍由 Firebase 登入與 Firestore 規則保護。

啟用此功能時，請依序：

1. 將本機 `統一後端.gs` 全文貼到 Apps Script，儲存後重新部署既有網頁應用程式。
2. 將本機 `firestore.rules` 全文貼到 Firebase Console 的「Firestore Database → Rules」並發布。
3. 推送前端後，以教師建立一份測試作業，再以家長帳號從班群卡片開啟，確認能查看圖片與收回結果。

### 匯入班級看板明日事項

教師在「行事曆 → 班級事項 → 帶入看板事項」可依**目前選取日期**讀取班級看板資料。系統會合併該日期的排程、明日事項與交辦投稿；預覽時可取消勾選、改寫科目或內容、調整順序，並選擇「附加整合」（保留現有內容）或「取代目前內容」。確認後只會帶入指定日期的聯絡簿編輯區，不會直接發布，也不會反向修改班級看板。

此功能使用班級看板既有的公開唯讀資料，沒有新增帳密、指令碼屬性或私密金鑰。首次更新時，除了推送 `teacher.html` 外，仍須將本機 `統一後端.gs` 完整貼入 Apps Script、儲存並重新部署網頁應用程式；既有 `script.external_request` 權限已涵蓋讀取需求。
...
教師端的「調查 → 資料」仍保留教師專用的個人資料卡與全班欄位查詢。這些資料只在教師發出查詢時由 Apps Script 讀取，不會提供給家長端。個人資料卡也會讀取 `PROFILE_PHOTO_FOLDER_NAME`（目前為 `2026_08_28`）資料夾中，同一座號的最新照片縮圖；相簿資料夾維持私有，不必設為公開分享。

請在既有試算表建立（或維持）名為「資料」的分頁：

1. 第一列填入欄位名稱，並保留一個名稱完全為「學號」的欄位；目前可放在第一欄。
2. 每位學生一列，學號必須與「名單」分頁的學號一致。其餘欄位可尚未填寫。
3. 日後直接在第一列右側新增欄位與資料即可；系統下次開啟資料查詢時會自動辨識，不必修改前端程式。
4. 將本機 `統一後端.gs` 完整貼入 Apps Script 後重新部署網頁應用程式；前端 `teacher.html` 則隨 GitHub Pages 更新。

### 家長查詢資料（需手動更新 Apps Script）

教師在「調查 → 資料 → 家長查詢資料」按「新增」後，可建立兩種資料：

1. **全班可查**：例如教師電話、辦公室分機；全班家長都會在「查詢」分頁看到同一內容。
2. **僅限本人查詢**：例如 OPEDID；以「學號 Tab 內容」為一行從試算表直接貼上，系統會依目前登入的學號只回傳該家長自己的內容。

系統首次由教師開啟這個功能時，會在同一份班級試算表自動建立「家長查詢」分頁。這個分頁是系統管理用資料，請從教師端新增、修改或刪除；個人資料不會寫入 Firebase，也不會下載到其他家長的裝置。啟用前請將新版 `統一後端.gs` 完整貼入 Apps Script、儲存並重新部署網頁應用程式；不需要更新 Firestore 規則。

### 線上請假單（需手動發布後端與規則）

家長送出請假單時，可填寫假別、起訖日期與節次，並以手寫方式完成家長簽章。系統會在 Google Drive 的聊天圖片根資料夾下建立 `學號 / leave / YYYY-MM-DD`，保存一份不對外分享的家長簽名 PNG；列印版採「高雄市立前峰國中學生請假單」欄位，包含班級、座號、姓名、假別、起訖時間、共計、家長簽章，以及導師、生教組、學務主任、校長與登記等後續紙本核章欄。列印使用的簽名資料受 Firestore 登入權限保護，僅該家長與教師可讀取。
...
表單管理位於教師端的「調查 → 表單」分頁。新題目預設為選擇題，每題提供至少兩個選項，也可自行新增選項或題目。若最後一個選項以「其他」開頭（例如「其他（請說明）」），家長選取後會出現必填的補充文字欄。教師先「儲存草稿」，可隨時按「修改草稿」與預覽；草稿不會被家長讀取。確認後按「發佈到班群」，系統才會公告並開放填寫，為避免已填回覆與題目不一致，發佈後不能再修改題目。按「預覽」可在儲存或發佈前直接查看家長端的填寫畫面；預覽不會建立表單、公告或儲存任何資料。

教師發佈後，家長會在班群看到「開啟並填寫」按鈕。教師也可按「產生連結」複製個別表單網址，用於其他公告管道；家長開啟該網址而尚未登入時，會先前往統一登入頁，登入後自動回到原表單。表單以手機逐題方式呈現，提供進度、上一步／下一步與最後的答案確認，再送出回覆。每位家長只有一份回覆紀錄，重新送出會更新自己的同一份內容；教師可看到已回覆／未回覆人數、名單與各份答案。表單連結會檢查既有登入身分，自動帶入座號與姓名。教師可「收回表單」：班群公告會顯示訊息已收回，家長停止填寫，既有回覆則保留供教師查看；刪除表單才會一併永久刪除所有回覆。

首次使用前，請將新版 `firestore.rules` 全部貼到 Firebase Console 的「Firestore Database → Rules」並發布，否則表單會因權限不足無法建立或送出。

### B表輔導紀錄（需手動發布 Firebase 規則）

教師端「記錄 → B表」的內容獨立儲存在 Firestore，家長端沒有入口，且規則只允許教師身分讀寫。啟用前請將本機 `firestore.rules` 全部貼到 Firebase Console 的「Firestore Database → Rules」並發布；未發布規則時，B表會顯示權限不足而無法儲存。
