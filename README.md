...
- 教師端「記錄」分為「相簿」、「作業資料」與「B表」；作業資料可一次上傳多張圖片，系統會整理為單一作業頁並只在班群發出一張入口卡片，避免大量圖片洗版。B表以日期時間、節次、方式、對象、資料來源、事件經過、處理情形與後續追蹤建立學生輔導紀錄，可篩選、修改、刪除，並能依學生與日期範圍輸出 A4 B表列印版或另存 PDF。
...
教師在「記錄 → 作業資料」填入標題，可一次選多張圖片；完成後班群只會出現一張「查看作業」卡片。家長必須先登入，才能透過卡片開啟作業資料頁。已發佈的作業可按「編輯」修改標題、科目、說明與截止日期，並可**追加**多張圖片；班群保留原本同一張入口卡片，不會重新發送。教師也能按「收回」，立即關閉家長頁並將班群卡片改為「訊息已收回」。
...
教師在「行事曆 → 班級事項 → 帶入明日事項」可讀取班級看板標示日期的資料。若班級看板已設定 `taskDate`，系統會優先使用該日期；只有來源未標示有效日期時，才會依不上課日避開週末與假日並推算下一個上課日。系統會合併其排程與明日交辦事項；預覽時可取消勾選、改寫科目或內容、調整順序。確認後只會附加到指定日期的聯絡簿編輯區，不會直接發布，也不會反向修改班級看板。
