...
- 聯絡簿可由學生姓名開啟個人資料卡，也可依欄位查詢全班資料；資料僅在教師查詢時從 Google 試算表讀取，不寫入 Firebase。個人資料卡另會以手機縮圖顯示當日相簿中該學生的照片。
...
教師端的「聯絡簿」提供兩種查詢：點選學生姓名開啟個人資料卡，或按「資料查詢」列出全班的單一欄位。資料只在教師發出查詢時由 Apps Script 讀取，不會寫入 Firebase 或提供給家長端。個人資料卡也會讀取當日相簿資料夾中，同一座號的照片縮圖；相簿資料夾維持私有，不必設為公開分享。
