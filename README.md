# 班級聯絡簿（807）

手機優先的班級聯絡與行事曆網站，用來集中處理每日公告、簽閱、請假、訊息、相簿與緊急語音通話，作為班級 LINE 群組以外的正式聯絡入口。

## 網站與管理入口

| 用途 | 網址 |
| --- | --- |
| 使用中的班級聯絡簿 | [GitHub Pages](https://suyungsheng-qfm.github.io/class-contact-book/) |
| 家長安裝與通知說明 | [操作說明頁](https://suyungsheng-qfm.github.io/class-contact-book/guide.html) |
| 教師相簿牆（電腦瀏覽／下載） | [教師相簿牆](https://suyungsheng-qfm.github.io/class-contact-book/gallery.html) |
| 原始碼與版本紀錄 | [GitHub 儲存庫](https://github.com/suyungsheng-qfm/class-contact-book) |
| 前端自動發布紀錄 | [GitHub Actions](https://github.com/suyungsheng-qfm/class-contact-book/actions) |
| GitHub Pages 設定 | [Pages 設定](https://github.com/suyungsheng-qfm/class-contact-book/settings/pages) |
| Firebase 專案管理 | [Firebase Console](https://console.firebase.google.com/project/classcontact-c148d/overview) |
| Apps Script 管理 | [Google Apps Script](https://script.google.com/home) |
| 語音通話 TURN／STUN 服務 | [Metered.ca](https://www.metered.ca/) |
| 學校公開行事曆 | [Google Calendar](https://calendar.google.com/calendar/embed?src=qisho218odg6vcgd3up3dpp6qg%40group.calendar.google.com&ctz=Asia%2FTaipei) |

> GitHub Pages 為唯一前端來源；Apps Script 只處理後端服務，不再放置或發布 HTML 前端。

## 功能

### 家長端

- 統一入口登入後自動進入家長頁面；驗證碼欄位支援手機數字鍵盤。
- 以月曆查看每日公告、班級／學校事項、假日與簽閱狀態；可回到今天。
- 三步內完成「今天公告 → 簽閱 → 班級訊息／私訊 → 圖片」的日常流程。
- 可在指定日期填寫線上請假單、手寫家長簽名；送出時會自動私訊通知導師，簽名與資料受登入權限保護。
- 班群與教師私訊皆支援文字、圖片、未讀提醒、送出中／失敗提示及重試；可由右上角鈴鐺主動開啟背景通知。
- 自己送出的文字或圖片可軟收回，畫面改顯示「訊息已收回」。
- 可向教師提出五分鐘內有效的緊急單一語音通話申請。
- 可透過手機瀏覽器加入桌面，作為網頁捷徑使用。
- 從班群的表單卡片開啟回覆頁時，會自動帶入登入身分，不需再次輸入學號或姓名。
- 「查詢」分頁可查看老師建立的資料；資料可設定為全班可查，或只允許目前登入家長查看自己的內容。

### 教師端

- 同一登入入口依教師帳號自動分流。
- 行事曆採月覽與當日事項分區：新增、修改、刪除、設定假日，以及一鍵設定當月週六／週日為假日。
- 可讀取學校行事曆，挑選匯入後仍能獨立修改班級行事曆資料。
- 班級事項可讀取 [807 班級看板](https://github.com/qfm-11407/classboard-807) 的下一個上課日「明日事項」；匯入前可勾選、修改與調整順序，確認後只附加到聯絡簿編輯區，仍須由教師自行儲存發布。
- 聯絡簿集中顯示待簽、已簽、未簽與請假統計；未簽採紅色提示，可篩選名單並快速私訊。
- 「調查」分為「表單」與「資料」；資料分頁可建立家長查詢項目、設定「全班可查」或「僅限本人」、以試算表格式批次貼上個人資料，並保留教師專用的全班欄位查詢與學生個人資料卡。個人資料卡可顯示指定相簿資料夾內的學生大頭照縮圖。
- 可從請假名單開啟含家長簽名的假單並列印；導師簽名欄保留給紙本手寫。
- 班群與個別家長私訊支援圖片、未讀提醒、失敗重試及軟收回；可由右上角鈴鐺主動開啟背景通知。
- 相簿支援教師以 iPhone 等手機單張上傳；上傳成功後會回到可再次上傳的初始狀態。
- 提供獨立的「教師相簿牆」網址：可在電腦依日期瀏覽相片、搜尋座號／檔名、開啟 Google Drive 原始檔或下載；不會出現在教師端功能列，且只限教師登入。
- 教師端「班務」分為「聯絡簿」與「作業」兩個子頁：聯絡簿集中簽閱統計；作業可多次選取圖片並持續累加，整理為單一作業頁且只在班群發出一張入口卡片，避免大量圖片洗版。「記錄」保留「相簿」與「B表」；B表以日期時間、節次、方式、對象、資料來源、事件經過、處理情形與後續追蹤建立學生輔導紀錄，可篩選、修改、刪除，並能依學生與日期範圍輸出 A4 B表列印版或另存 PDF。
- 可從家長的通話申請發起一對一網頁語音通話。
- 可建立並修改表單草稿、以家長視角預覽、發佈到班群、查看家長回覆、關閉或完整刪除表單；查看回覆時可將表單卡片私訊給未回覆的家長。

## 系統架構

| 元件 | 職責 |
| --- | --- |
| GitHub Pages | 發布 `index.html`、`guardians.html`、`teacher.html` 與 PWA 資源。推送 `main` 後自動更新。 |
| Firebase Firestore | 儲存各學期的聯絡簿、行事曆、簽閱／請假、班群、私訊、相簿資料與未讀狀態，並提供即時同步。 |
| Firebase Cloud Messaging | 在手機捷徑不開啟時，傳送班群與私訊的背景通知；iPhone／iPad 支援時可在捷徑上顯示未讀徽章。 |
| Google Apps Script | 統一登入、教師／家長權限判斷、核發短效 Firebase 自訂憑證、圖片上傳、學校行事曆與班級看板明日事項讀取，以及語音通話訊號交換。 |
| Google 試算表 | 保存家長帳號、驗證碼、學生名單，以及教師限定的學生個人資料。 |
| Google Drive | 保存班級相簿、聊天圖片、作業圖片，以及按學生與日期歸檔的私密請假簽名。 |
| Metered.ca TURN／STUN 服務 | 目前的語音通話連線服務，協助教師與單一家長建立 WebRTC 音訊連線；僅用於緊急情境。 |

目前前端依日期自動選擇 `114-02`、`115-01` 或 `115-02` 對應的 Firebase 學期資料。聊天資料跨學期共用，以保留對話紀錄。

## 從零開始的系統開發流程（新手手冊）

這個專案不是一次做完的網站，而是從「家長每天需要看公告、簽閱、傳訊息」這條核心流程開始，再逐步加入請假、相簿、表單、資料查詢、作業與通知。新手也可以用同樣的方法，從一個實際需要解決的問題出發，建立可長期使用的系統。

### 1. 先定義問題，不要先決定畫面

先用一句話寫出系統要取代或改善的事情。例如本專案的起點是：

> 讓家長可以在手機上完成「看今天公告、簽閱、聯絡老師、查看圖片」，減少分散在 LINE、紙本和口頭通知的資訊。

接著寫出最小可用流程（MVP）。這個專案的第一版只有四步：登入、今日公告、簽閱、班級訊息。只有在這條流程可用後，才陸續加入請假、相簿、表單、成績與語音通話。

每一項新功能都建議先依這個格式描述：

```text
功能名稱：例如「作業資料」
誰使用：教師建立；家長查看
使用時機：教師要一次提供多張作業圖片，但不希望班群被大量圖片洗版
完成流程：教師建立一份作業 → 選圖、排序、填說明 → 發佈一張班群入口卡 → 家長登入後查看
資料與權限：只有教師可建立、修改、收回；家長只可讀取已發佈的內容
例外情況：圖片上傳失敗可重試；已發佈後新增圖片不重複發班群卡；收回後家長不可再開啟
完成標準：教師與家長各用手機完成一次，班群只出現一張卡片，收回後無法讀取
```

這種寫法比「做一個作業上傳頁」更容易得到正確結果，因為它同時說明了使用者、資料、例外和驗收方式。

### 2. 先畫出角色、資料與權限

在寫程式前，先回答三個問題：誰能建立？誰能看？資料放在哪裡？本專案的分工如下，可作為同類系統的參考。

| 類型 | 建立／修改者 | 可查看者 | 保存位置 |
| --- | --- | --- | --- |
| 聯絡簿、行事曆 | 教師 | 全班已登入家長 | Firebase Firestore |
| 簽閱、請假、私訊 | 對應家長與教師 | 該家長、教師 | Firestore；請假簽名在私有 Google Drive |
| 班群、表單、作業 | 教師 | 已登入家長 | Firestore；圖片在 Google Drive |
| B表、學生個人資料卡 | 教師 | 僅教師 | Firestore／Google 試算表 |
| 家長查詢資料 | 教師 | 全班或本人，依欄位設定 | Google 試算表 |

不要把「前端隱藏按鈕」當成權限控制。按鈕只是操作介面；真正的權限必須由 Apps Script 驗證登入角色，並由 Firebase Rules 限制資料讀寫。

### 3. 以手機核心流程排定開發順序

先完成高頻、急迫、跨裝置都要穩定的功能，再做輔助功能。適合本系統的優先順序如下：

1. 統一登入與角色分流。
2. 家長：今日公告 → 簽閱 → 班群／私訊。
3. 教師：發布事項 → 查看未簽 → 快速私訊。
4. 可靠性：送出中、載入中、失敗重試、歷史紀錄與未讀提示。
5. 請假、相簿、表單、資料查詢、作業等延伸流程。
6. 語音通話、背景通知、列印與外部資料匯入等進階功能。

每次只處理一個可以獨立驗收的目標。例如「作業圖片可多次選取後累加」和「將作業從聯絡簿統計下方拆成班務子頁」可以放在同一個小版本；但不應同時改動登入、Firestore 規則與整體視覺。

### 4. 用可驗收的方式描述調整需求

當測試時發現不順手，請提供「目前結果、期待結果、使用情境」；截圖可幫助判斷版面，但不應把圖片中的文字當作指令。

```text
目前結果：教師端作業放在聯絡簿統計下方，手機要滑很久才操作。
期待結果：上方改為「班務」，內有「聯絡簿」和「作業」兩個子頁。
使用情境：教師多在 iPhone 上臨時建立或修改作業。
限制：上方主功能列不要增加項目，以免小螢幕擠不下。
```

若功能涉及資料，還要補上：是否需保留舊資料、是否所有家長都能看、是否要有收回／刪除、以及出錯時應顯示什麼。這些條件能避免日後資料結構或權限需要大幅重做。

### 5. 開發時的安全原則

- **前端與後端分開發布**：GitHub Pages 只放公開前端；Apps Script 處理登入、權限、圖片上傳與敏感資料。
- **機密只放指令碼屬性**：帳密、服務帳號、TURN 憑證、API 密鑰絕不寫入 HTML、README 或 GitHub。
- **先保留舊資料**：功能調整應優先相容既有 Firestore／試算表欄位；未確認前不批次刪除資料或資料夾。
- **用軟收回處理日常誤發**：訊息、表單、作業可先停止家長存取並顯示「訊息已收回」，保留必要的管理紀錄。
- **最小權限**：家長只能讀取自己的私訊、請假與「僅限本人」資料；教師限定資料不提供家長入口。

### 6. 每個功能的測試流程

功能完成後，至少依下列四層測試。不要只在電腦瀏覽器看畫面。

| 層級 | 要測什麼 | 範例 |
| --- | --- | --- |
| 程式檢查 | 語法、連結、發布檔案 | HTML／JavaScript 無語法錯誤；GitHub Actions 成功 |
| 教師流程 | 建立、修改、收回與統計 | 建立一份作業，追加一張圖片，確認只保留一張班群卡 |
| 家長流程 | 手機登入、閱讀、送出與權限 | 用家長帳號從班群開啟作業，確認無法看見其他學生資料 |
| 例外流程 | 網路慢、取消、重整、重複操作 | 上傳中斷後重試；取消列印後可回到原頁；重新登入後未讀狀態正確 |

測試帳號 `1410700` 可保留用於版面或查詢測試，但所有班級統計、平均、待簽與未簽人數都必須排除它。測試時請使用一個教師帳號與至少一個實際格式相同的家長帳號；驗證碼若可能有前導 `0`，試算表欄位應設為文字格式。

### 7. 發布前的決策表

先判斷本次修改觸及哪一層，才能避免「前端已更新但後端仍是舊版」的錯誤。

| 本次改動 | 要推送 GitHub Pages | 要更新 Apps Script | 要發布 Firestore Rules |
| --- | --- | --- | --- |
| 純版面、文字、互動調整 | 是 | 否 | 否 |
| 新增前端資料欄位但沿用既有 API | 是 | 視情況 | 視情況 |
| 登入、權限、試算表讀寫、圖片上傳、外部資料讀取 | 通常是 | 是，儲存並重新部署 | 視讀寫規則而定 |
| 新集合、表單／B表／作業／請假資料讀寫 | 是 | 視功能而定 | 是 |

發布順序建議為：先更新後端與 Rules（若需要）→ 用教師測試 → 推送前端到 `main` → 等 GitHub Actions 成功 → 用家長測試。遇到登入失敗時，先檢查 Apps Script 部署與 OAuth／IAM 授權；遇到資料讀寫失敗時，再檢查 Firebase Rules，而不是直接把資料庫改成公開。

### 8. 實際回饋與迭代循環

以下循環就是本專案從對話中逐步形成的方式：

```text
觀察真實操作
  → 用一句話描述卡住的位置與期望
  → 先做最小調整
  → 教師、家長各測一次
  → 確認資料與權限沒有外洩
  → 推送前端／部署後端
  → 在真實手機上再確認
```

提出回饋時，附上「角色（教師／家長）」、「裝置與瀏覽器」、「操作步驟」、「實際畫面」、「預期結果」會很有效。例如「小米平板直式首次開啟時，看不到底部功能列；旋轉後才出現」這類描述可以直接導向版面與視窗高度測試，而不是只說「平板壞掉」。

### 9. 新手每次開發前後的檢查清單

開發前：確認要解決的單一流程、使用角色、資料位置、權限範圍、完成標準，以及是否需要保留舊資料。

開發後：確認手機直式與平板、教師與家長、成功與失敗重試、重新登入、未讀提示、隱私權限；再確認此次是否需更新 Apps Script、Firebase Rules，最後才推送發布。

這個流程的重點不是一次把功能想得很完整，而是每次都能清楚知道「誰在什麼情況下做什麼、資料會去哪裡、如何算完成」。只要守住這四件事，系統就能安全地持續擴充。

## 專案檔案

| 檔案 | 說明 |
| --- | --- |
| `index.html` | 統一登入頁，依身份導向教師或家長端。 |
| `guardians.html` | 家長端介面與 Firebase 即時資料處理。 |
| `teacher.html` | 教師端介面與日常管理功能。 |
| `forms.html` | 家長由班群連結開啟的表單回覆頁。 |
| `homework.html` | 家長由班群入口卡開啟的作業圖片頁。 |
| `gallery.html` | 教師專用的獨立電腦相簿牆，可瀏覽與下載既有相簿照片。 |
| `manifest.webmanifest`、`service-worker.js` | 安裝為手機桌面捷徑、基本離線快取、背景推播與捷徑徽章。 |
| `app-icon-*`、`apple-touch-icon.png`、`807.png` | 網站與桌面捷徑圖示資源。 |
| `yssu.png` | 教師專用頭像，用於家長端的班群與私訊教師訊息。 |
| `統一後端.gs`（本機／Apps Script） | Apps Script 後端程式；不作為 GitHub Pages 前端部署內容。 |
| `firestore.rules` | Firebase Firestore 的存取規則；發布前請貼入 Firebase Console。 |

## 發布與維護

### 前端

1. 修改前端檔案後提交並推送至 `main`。
2. GitHub Actions 會自動發布 GitHub Pages。
3. 在 Actions 確認工作流程成功後，重新整理網站即可取得新版。

### Apps Script 後端

後端程式僅在帳密驗證、上傳、行事曆讀取或語音通話邏輯變更時才需要更新。部署為網頁應用程式時，採用「以部署者身分執行」與「任何人」可存取，前端透過既有 API 位址呼叫。

Apps Script 的「指令碼屬性」至少需要設定：

- `TEACHER_ACCOUNT`
- `TEACHER_PASSWORD`
- `VOICE_TURN_URLS`（Metered.ca 提供的 TURN 位址）
- `VOICE_TURN_USERNAME`（Metered.ca 使用者名稱）
- `VOICE_TURN_CREDENTIAL`（Metered.ca 密鑰）
- `FIREBASE_SERVICE_ACCOUNT_EMAIL`（Firebase／Google Cloud 服務帳號的電子郵件）
- `FCM_VAPID_KEY`（Firebase Cloud Messaging 的 Web Push 公開金鑰）

請只在 Apps Script 指令碼屬性設定實際帳密與 TURN 憑證，勿將它們寫入 HTML、README 或 GitHub。

### 教師相簿牆（需手動發布後端）

電腦可直接開啟 [教師相簿牆](https://suyungsheng-qfm.github.io/class-contact-book/gallery.html)。首次開啟會要求教師透過統一登入頁驗證；登入完成後，頁面會依既有相簿日期資料夾顯示照片牆，並提供座號／檔名搜尋、「開啟」Google Drive 原始檔與「下載」連結。日期可多選，也可切換為「排除勾選日期」；為避免一次載入過多縮圖，單次最多顯示 12 個日期。這個網址不放在教師端的功能列，方便在電腦另開分頁使用。

相簿資料夾維持私有，不需要設定公開分享。縮圖由 Apps Script 以教師工作階段讀取，只有已驗證的教師瀏覽器會收到資料。請將本機 `統一後端.gs` 完整貼入 Apps Script、儲存並重新部署既有網頁應用程式，才能啟用相簿牆讀取功能；既有 Google Drive 權限已足夠，不需要新增 API 或指令碼屬性。

### 作業資料（需手動發布後端與規則）

教師在「班務 → 作業」填入標題，可一次選多張圖片，也可每次只選一張、持續累加；完成後班群只會出現一張「查看作業」卡片。家長必須先登入，才能透過卡片開啟作業資料頁。每張圖片可填寫說明，並以向上／向下按鈕調整顯示順序；家長頁會依相同順序顯示說明。已發佈的作業可按「編輯」修改標題、科目、說明與截止日期，並可**追加**多張圖片；班群保留原本同一張入口卡片，不會重新發送。教師也能按「收回」，立即關閉家長頁並將班群卡片改為「訊息已收回」。

作業圖片會放在班級相簿根資料夾下的 `作業資料 / 作業識別碼` 資料夾。為了讓各型手機的網頁能直接載入圖片，這些**圖片檔案**採「知道連結者可檢視」；請勿上傳學生個資、成績、聯絡資訊或任何不適合被轉傳的內容。作業標題、說明與入口頁本身仍由 Firebase 登入與 Firestore 規則保護。

啟用此功能時，請依序：

1. 將本機 `統一後端.gs` 全文貼到 Apps Script，儲存後重新部署既有網頁應用程式。
2. 將本機 `firestore.rules` 全文貼到 Firebase Console 的「Firestore Database → Rules」並發布。
3. 推送前端後，以教師建立一份測試作業，再以家長帳號從班群卡片開啟，確認能查看圖片與收回結果。

### 匯入班級看板明日事項

教師在「行事曆 → 班級事項 → 帶入看板事項」可依**目前選取日期**讀取班級看板資料。系統會合併該日期的排程、明日事項與交辦投稿；預覽時可取消勾選、改寫科目或內容、調整順序，並選擇「附加整合」（保留現有內容）或「取代目前內容」。確認後只會帶入指定日期的聯絡簿編輯區，不會直接發布，也不會反向修改班級看板。

此功能使用班級看板既有的公開唯讀資料，沒有新增帳密、指令碼屬性或私密金鑰。首次更新時，除了推送 `teacher.html` 外，仍須將本機 `統一後端.gs` 完整貼入 Apps Script、儲存並重新部署網頁應用程式；既有 `script.external_request` 權限已涵蓋讀取需求。

### 背景通知與桌面捷徑徽章（需手動啟用）

班群或私訊送出後，系統會由 Apps Script 以 Firebase Cloud Messaging 通知已同意接收通知的裝置。為避免在鎖定畫面顯示私人內容，通知僅顯示「班群有新訊息」、「老師傳來新訊息」或「家長傳來新訊息」，不含正文與圖片。已不再使用的裝置通知識別會在傳送失敗時自動移除；試算表會自動新增「通知訂閱」分頁保存通知識別、角色與更新時間。

啟用步驟：

1. 在 Firebase Console 開啟 **專案設定 → Cloud Messaging → Web configuration → Web Push certificates**，按「Generate key pair」，複製顯示的**公開金鑰**。這不是服務帳戶私密金鑰，也可以安全地交給網頁使用。
2. 在 Apps Script 的「專案設定 → 指令碼屬性」新增 `FCM_VAPID_KEY`，貼上第 1 步公開金鑰。
3. 在 Firebase 專案的 Google Cloud Console 確認已啟用 **Firebase Cloud Messaging API** 與 **FCM Registration API**；新版專案通常已啟用，但舊專案可能需要手動啟用。
4. 將本機 `統一後端.gs` 與 `appsscript.json` 完整更新到 Apps Script，儲存後手動執行 `requestFirebaseMessagingAuthorization` 並以部署者帳號完成新的授權流程，再重新部署網頁應用程式。`appsscript.json` 要保留 `cloud-platform` 與 `firebase.messaging` scope。
5. 前端推送至 GitHub Pages 後，在教師與家長手機各自登入，點右上角鈴鐺並允許通知。iPhone／iPad 必須以「加入主畫面」建立的捷徑開啟，才可使用網頁推播與捷徑徽章。
6. 以另一個帳號傳送班群或私訊測試：App 開啟時會同步正確未讀數；關閉時的推播先以 `1` 作提醒，重新開啟後會依目前未讀狀態清除或更新徽章。

Firebase Cloud Messaging 本身不收費；此版本直接由既有 Apps Script 發送，不需要 Cloud Functions 或額外伺服器。

### 學生個人資料（需手動更新 Apps Script）

教師端的「調查 → 資料」仍保留教師專用的個人資料卡與全班欄位查詢。這些資料只在教師發出查詢時由 Apps Script 讀取，不會提供給家長端。個人資料卡也會讀取 `PROFILE_PHOTO_FOLDER_NAME`（目前為 `2026_08_28`）資料夾中，同一座號的最新照片縮圖；相簿資料夾維持私有，不必設為公開分享。

請在既有試算表建立（或維持）名為「資料」的分頁：

1. 第一列填入欄位名稱，並保留一個名稱完全為「學號」的欄位；目前可放在第一欄。
2. 每位學生一列，學號必須與「名單」分頁的學號一致。其餘欄位可尚未填寫。
3. 日後直接在第一列右側新增欄位與資料即可；系統下次開啟資料查詢時會自動辨識，不必修改前端程式。
4. 將本機 `統一後端.gs` 完整貼入 Apps Script 後重新部署網頁應用程式；前端 `teacher.html` 則隨 GitHub Pages 更新。

### 家長查詢資料（需手動更新 Apps Script）

教師在「調查 → 資料 → 家長查詢資料」按「新增」後，可建立兩種資料：

1. **全班可查**：例如教師電話、辦公室分機；全班家長都會在「查詢」分頁看到同一內容。
2. **僅限本人查詢**：例如 OPEDID；以「學號 Tab 內容」為一行從試算表直接貼上，系統會依目前登入的學號只回傳該家長自己的內容。

系統首次由教師開啟這個功能時，會在同一份班級試算表自動建立「家長查詢」分頁。這個分頁是系統管理用資料，請從教師端新增、修改或刪除；個人資料不會寫入 Firebase，也不會下載到其他家長的裝置。啟用前請將新版 `統一後端.gs` 完整貼入 Apps Script、儲存並重新部署網頁應用程式；不需要更新 Firestore 規則。

### 線上請假單（需手動發布後端與規則）

家長送出請假單時，可填寫假別、起訖日期與節次，並以手寫方式完成家長簽章。系統會在 Google Drive 的聊天圖片根資料夾下建立 `學號 / leave / YYYY-MM-DD`，保存一份不對外分享的家長簽名 PNG；列印版採「高雄市立前峰國中學生請假單」欄位，包含班級、座號、姓名、假別、起訖時間、共計、家長簽章，以及導師、生教組、學務主任、校長與登記等後續紙本核章欄。列印使用的簽名資料受 Firestore 登入權限保護，僅該家長與教師可讀取。

啟用前請：

1. 將本機 `統一後端.gs` 完整貼到 Apps Script，重新部署網頁應用程式。
2. 將本機 `firestore.rules` 全部貼到 Firebase Console 的「Firestore Database → Rules」並發布。
3. 以家長建立一筆未來日期的請假單，確認教師端「聯絡簿 → 請假」可按「假單」開啟列印視窗。

## Firebase 權限更新（需手動操作）

這項更新會停止匿名 Firebase 存取。完成前端發布前，請依下列順序操作，以避免家長頁面無法載入。

1. 在 Apps Script 的「專案設定 → Google Cloud Platform (GCP) 專案」改綁到 Firebase 專案的**專案編號**（不是 `classcontact-c148d` 這個專案 ID）。讓 Apps Script 與 Firebase 使用同一個 Google Cloud 專案，避免 API 啟用在不同專案而造成 403。
2. 在 Google Cloud 的「Google Auth Platform」完成 OAuth 設定。於「品牌」填入應用程式名稱、支援信箱與開發人員聯絡信箱；若要避免「測試中」授權每 7 天失效，還需填入首頁 `https://suyungsheng-qfm.github.io/class-contact-book/`、隱私權政策 `https://suyungsheng-qfm.github.io/class-contact-book/privacy.html`，並在已授權網域填入 `suyungsheng-qfm.github.io`。接著在「目標對象」選「外部」並按「發布應用程式」。這只影響部署者的 Google 授權週期，家長不需加入測試使用者，也不會取得 Google 權限。
3. 在這個 Google Cloud 專案的「API 和服務」啟用 **IAM Service Account Credentials API**。
4. 選擇 Firebase 專案的服務帳號，記下其電子郵件（常見為 `firebase-adminsdk-…@classcontact-c148d.iam.gserviceaccount.com`）；不需建立或下載私密金鑰。
5. 在該服務帳號的 IAM 權限中，授予實際部署 Apps Script 的 Google 帳號 **Service Account Token Creator** 角色。
6. 在 Apps Script 開啟本機的 `統一後端.gs`，以檔案內容完整取代現有後端程式；再開啟 `appsscript.json` 並套用其中的 `oauthScopes`，其中必須包含 `https://www.googleapis.com/auth/cloud-platform`。
7. 在 Apps Script 的「專案設定 → 指令碼屬性」新增 `FIREBASE_SERVICE_ACCOUNT_EMAIL`，填入第 4 步的服務帳號電子郵件。
8. 在 Apps Script 編輯器手動依序執行 `requestCloudPlatformAuthorization` 與 `testFirebaseIamSigning`。後者顯示 `IAM signing succeeded.` 才代表 OAuth、IAM API 與服務帳戶權限都已完成。
9. 重新部署 Apps Script 網頁應用程式：選擇「管理部署 → 編輯」，建立新版本；設定為「以我身分執行」及「任何人」可存取。部署網址維持既有 API 位址即可。
10. 確認教師與一位家長可正常登入 GitHub Pages 後，在 Firebase Console 的「Firestore Database → Rules」貼上 `firestore.rules` 全部內容並發布。這一步後，匿名使用者會立刻失去資料庫存取權。
11. 以教師、家長各測試一次後，在 Firebase Console 的「Authentication → Sign-in method」停用 Anonymous。

若曾為本項目下載服務帳號 JSON 私密金鑰，請在 Google Cloud Console 將該金鑰撤銷，並自本機安全刪除；這個架構不會使用它。

### 特別注意：OAuth 測試模式會每 7 天失效

若 Google Cloud 的「Google Auth Platform → 目標對象」維持在「測試中」，部署 Apps Script 的 Google 帳號所使用的 OAuth 授權可能在 7 天後失效，造成家長與教師都無法登入，且必須回 Apps Script 重新授權。這不是家長驗證碼或 Firebase 資料庫過期。

正式使用前，請將發布狀態改為「實際運作中／In production」。新版 Google Auth Platform 可能同時要求在「品牌」完成應用程式名稱、支援信箱、開發人員聯絡信箱、首頁、隱私權政策與已授權網域設定；本專案使用的網址為：

- 首頁：`https://suyungsheng-qfm.github.io/class-contact-book/`
- 隱私權政策：`https://suyungsheng-qfm.github.io/class-contact-book/privacy.html`
- 已授權網域：`suyungsheng-qfm.github.io`

改為正式環境後，請以 `qfmstudy01@gmail.com` 到 Google 帳戶的「第三方應用程式連線」撤銷本 Apps Script 專案的舊授權，再回 Apps Script 執行 `requestFirebaseMessagingAuthorization` 並重新允許，最後執行 `testFirebaseIamSigning` 確認顯示 `IAM signing succeeded.`。只有新授權完成後，才能確實解除既有測試授權的 7 天期限。撤銷與重新授權的短暫期間，使用者可能無法登入，請選擇無人使用的時段操作。

即使已改為正式環境，系統仍會在 6 小時後要求重新輸入班級聯絡簿帳密；這是後端登入工作階段的正常安全設計，和 Google OAuth 重新授權不同。

完成後的驗證重點：未登入直接開啟 `guardians.html` 或 `teacher.html` 應回到統一登入頁；家長僅能看到自己的簽閱／請假與私訊；教師仍可看到全班統計與全部私訊。登入後的 Firebase 憑證由 Apps Script 核發、有效期約一小時，瀏覽器關閉後也不會保留登入狀態。

### 班級表單

表單管理位於教師端的「調查 → 表單」分頁。新題目預設為選擇題，每題提供至少兩個選項，也可自行新增選項或題目。若最後一個選項以「其他」開頭（例如「其他（請說明）」），家長選取後會出現必填的補充文字欄。教師先「儲存草稿」，可隨時按「修改草稿」與預覽；草稿不會被家長讀取。確認後按「發佈到班群」，系統才會公告並開放填寫，為避免已填回覆與題目不一致，發佈後不能再修改題目。按「預覽」可在儲存或發佈前直接查看家長端的填寫畫面；預覽不會建立表單、公告或儲存任何資料。

教師發佈後，家長會在班群看到「開啟並填寫」按鈕。教師也可按「產生連結」複製個別表單網址，用於其他公告管道；家長開啟該網址而尚未登入時，會先前往統一登入頁，登入後自動回到原表單。表單以手機逐題方式呈現，提供進度、上一步／下一步與最後的答案確認，再送出回覆。每位家長只有一份回覆紀錄，重新送出會更新自己的同一份內容；教師可看到已回覆／未回覆人數、名單與各份答案。表單連結會檢查既有登入身分，自動帶入座號與姓名。教師可「收回表單」：班群公告會顯示訊息已收回，家長停止填寫，既有回覆則保留供教師查看；刪除表單才會一併永久刪除所有回覆。

首次使用前，請將新版 `firestore.rules` 全部貼到 Firebase Console 的「Firestore Database → Rules」並發布，否則表單會因權限不足無法建立或送出。

### B表輔導紀錄（需手動發布 Firebase 規則）

教師端「記錄 → B表」的內容獨立儲存在 Firestore，家長端沒有入口，且規則只允許教師身分讀寫。啟用前請將本機 `firestore.rules` 全部貼到 Firebase Console 的「Firestore Database → Rules」並發布；未發布規則時，B表會顯示權限不足而無法儲存。

## 使用提醒

- 教師與家長都只應使用自己的帳號登入。
- 收回訊息採「軟收回」：畫面隱藏原內容，但不刪除 Firebase 訊息文件或 Google Drive 圖片檔，以保留紀錄與降低誤刪風險。
- 語音通話為一位教師對一位家長，不含視訊、多人通話或錄音；建議僅用於緊急聯繫。
